...
部分激活函数是非零均值的，如$ReLU$, $Sigmoid$等激活函数，他会造成网络收敛很慢。我们可以简单看下表示式：$f=(w_{i} x _{i}+b)$，其中$x_{i}$为$sigmoid$函数的输出。那么，在计算损失函数后，需要进行反向传播更新该权重$w_{i}$。这时候，对$w_{i}$进行求导，是直接与$x_{i}$相关的，而因为$x_{i}$是大于$0$的值，所以这时候的梯度方向就会完全取决于$\frac{dL}{df}$。这时候若$\frac{dL}{df}$恒正或者恒为负，那么就会出现$zig-zagging$ $dynamics$的问题,使得网络收敛变慢。
...
$$
\begin{aligned}
g^{\prime}(z) &=(\frac{e^{z}-e^{-z}}{e^{z}+e^{-z}})^{\prime} \\
&=\frac{4}{(e^{z}+e^{-z})^{2}} \\
&=1-g(z)^{2}
\end{aligned}
$$
...
或者表达为：
$$f(\alpha, x)=\{\begin{array}{ll}\alpha(e^{x}-1) & \text { for } x \leq 0 \\ x & \text { for } x>0\end{array}.$$
...
当$\beta arrow \infty, \sigma(x)=(1+\exp (-x))^{-1}$ 为0或1. Swish变为ReLU: $f(x)=2 \max (0, x)$。
...
而上式函数是无法直接计算的，因此可以使用另外的方式来进行逼近，论文得到的表达式为：$0.5 x(1+\tanh [\sqrt{2 / \pi}(x+0.044715 x^{3})])$。或者为$GELU(x)=\frac{x}{1+e^{-1.702 x}}$。
